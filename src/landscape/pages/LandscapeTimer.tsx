...
function TimerPage({ routerState }: { readonly routerState: TimerRouterState }): React.ReactElement {
  ...
  return (
    <div className={cn('relative flex h-full bg-surface select-none', appStyle === 'paper' && 'paper-timer-page')}>
      ...
      {/* ── Left: ring / paper clock / pixel block ─────────────────── */}
      <div className="flex w-[55%] flex-col items-center justify-center relative">
        {appStyle === 'pixel-gemini' ? (
          ...
          <div className="flex flex-col items-center gap-6 w-full px-8">
            {flashComplete && (
              <p className="animate-[complete-enter_300ms_ease-out_forwards] text-success text-xl font-bold uppercase tracking-widest">{t('timer.complete')}</p>
            )}
            ...
            {!flashComplete && !flashNext && (
              <>
                <div className="font-mono text-7xl font-bold text-primary tracking-widest mb-4" style={{ textShadow: '4px 4px 0 var(--primary-container)' }}>
                  {timeLabel}
                </div>
                <div className="w-full max-w-[320px] border-4 border-outline bg-surface-variant p-1 shadow-[4px_4px_0px_0px_var(--primary-container)]">
                  <div 
                    className="h-4 transition-[width] duration-1000 ease-linear"
                    style={{ width: `${progress}%`, backgroundColor: colorHex }}
                  />
                </div>
                <div className="flex items-center gap-6 mt-4">
                  <button onClick={() => setShowStopDialog(true)} aria-label={t('timer.stopLog')} className="p-4 border-2 border-outline bg-error text-white shadow-[2px_2px_0px_0px_var(--outline)] active:translate-y-1 active:shadow-none transition-all">
                    <Square size={28} fill="currentColor" />
                  </button>
                  {phase === 'running' && (
                    <button onClick={() => { pauseTimer(); navigator.vibrate?.(10); }} aria-label={t('timer.pause')} className="p-4 border-2 border-outline bg-primary-container text-on-primary-container shadow-[4px_4px_0px_0px_var(--outline)] active:translate-y-1 active:shadow-[2px_2px_0px_0px_var(--outline)] transition-all">
                      <Pause size={28} fill="currentColor" />
                    </button>
                  )}
                  {phase === 'paused' && (
                    <button onClick={() => { resumeTimer(); navigator.vibrate?.(10); }} aria-label={t('timer.resume')} className="p-4 border-2 border-outline bg-primary text-on-primary shadow-[4px_4px_0px_0px_var(--outline)] active:translate-y-1 active:shadow-[2px_2px_0px_0px_var(--outline)] transition-all">
                      <Play size={28} fill="currentColor" />
                    </button>
                  )}
                  {isCombo && (
                    <button onClick={() => setShowSkipDialog(true)} disabled={!isSkipEnabled} aria-label={t('timer.skipProject')} className="p-4 border-2 border-outline bg-surface-variant text-on-surface-variant shadow-[2px_2px_0px_0px_var(--outline)] active:translate-y-1 active:shadow-none transition-all disabled:opacity-50">
                      <SkipForward size={28} />
                    </button>
                  )}
                </div>
